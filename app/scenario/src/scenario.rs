...
impl Scenario {
    ...
    pub fn workspace<P: AsRef<Path>>(workspace: P) -> Result<Self, E> {
        ...
        if !workspace.exists() {
            Err(E::PathDoesNotExist(workspace.to_string_lossy().to_string()))?;
        }
        Self::search(&workspace)?
            .map(|filepath| Self { filepath })
            .ok_or(E::ScenarioNotFound(workspace.to_string_lossy().to_string()))
    }
