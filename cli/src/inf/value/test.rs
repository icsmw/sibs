...
#[test]
fn vec() {
    for left in ValueRef::iter() {
        let inner = match left {
            ValueRef::bool => ValueRef::bool,
            ValueRef::Optional(..) => ValueRef::Optional(Box::new(ValueRef::Empty)),
            ValueRef::Repeated(..) => ValueRef::Repeated(Box::new(ValueRef::Empty)),
            ValueRef::OneOf(..) => ValueRef::OneOf(vec![ValueRef::Empty]),
            ValueRef::Empty => ValueRef::Empty,
            ValueRef::u8 => ValueRef::u8,
            ValueRef::u16 => ValueRef::u16,
            ValueRef::u32 => ValueRef::u32,
            ValueRef::u64 => ValueRef::u64,
            ValueRef::u128 => ValueRef::u128,
            ValueRef::usize => ValueRef::usize,
            ValueRef::i8 => ValueRef::i8,
            ValueRef::i16 => ValueRef::i16,
            ValueRef::i32 => ValueRef::i32,
            ValueRef::i64 => ValueRef::i64,
            ValueRef::i128 => ValueRef::i128,
            ValueRef::isize => ValueRef::isize,
            ValueRef::Numeric => ValueRef::Numeric,
            ValueRef::PathBuf => ValueRef::PathBuf,
            ValueRef::String => ValueRef::String,
            ValueRef::Vec(..) => ValueRef::Vec(Box::new(ValueRef::Empty)),
            ValueRef::Task(..) => ValueRef::Task(vec![], Box::new(ValueRef::Empty)),
        };
        let left = ValueRef::Vec(Box::new(inner.clone()));
        match &inner {
            ValueRef::bool => {
                assert!(ValueRef::Vec(Box::new(ValueRef::bool)).is_compatible(&left));
                assert!(
                    !ValueRef::Vec(Box::new(ValueRef::Optional(Box::new(ValueRef::Empty))))
                        .is_compatible(&left)
                );
                assert!(
                    !ValueRef::Vec(Box::new(ValueRef::Repeated(Box::new(ValueRef::Empty))))
                        .is_compatible(&left)
                );
                assert!(
                    !ValueRef::Vec(Box::new(ValueRef::OneOf(vec![ValueRef::Empty])))
                        .is_compatible(&left)
                );
                assert!(!ValueRef::Vec(Box::new(ValueRef::Empty)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::u8)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::u16)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::u32)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::u64)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::u128)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::usize)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::i8)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::i16)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::i32)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::i64)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::i128)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::isize)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::Numeric)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::PathBuf)).is_compatible(&left));
                assert!(!ValueRef::Vec(Box::new(ValueRef::String)).is_compatible(&left));
                assert!(
                    !ValueRef::Vec(Box::new(ValueRef::Vec(Box::new(ValueRef::Empty))))
                        .is_compatible(&left)
                );
                assert!(!ValueRef::Vec(Box::new(ValueRef::Task(
                    vec![],
                    Box::new(ValueRef::Empty)
                )))
                .is_compatible(&left));
            }
            ValueRef::Optional(..) => {
            ...
            ValueRef::Task(..) => {
                ...
                assert!(
                    ValueRef::Vec(Box::new(ValueRef::Task(vec![], Box::new(ValueRef::Empty))))
                        .is_compatible(&left)
                );
            }
        };
